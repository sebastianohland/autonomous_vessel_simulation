...
def calculate_collision_parameters(os_pos, os_cog, os_sog, os_len, ts_pos, ts_cog, ts_sog, ts_len):
    ...
    os_vel_pos = (os_pos[0] + os_vel[0], os_pos[1] + os_vel[1])
    ts_vel_pos = (ts_pos[0] + ts_vel[0], ts_pos[1] + ts_vel[1])

    # calculate bow cross position (intersecting lines)
    ts_bcr_pos = line_intersect(os_pos, os_vel_pos, ts_pos, ts_vel_pos)
    #print("TS bow cross pos x: {}   y: {}".format(round(ts_bcr_pos[0], 2), round(ts_bcr_pos[1], 2)))
    ...
    collision_parameters = np.around(collision_parameters, 2)
...
def calculate_state_collision_parameters(state):
    ...
    os_pos = (state["ownship"]["xpos"], state["ownship"]["ypos"])
    os_cog = state["ownship"]["cog"]
    os_sog = state["ownship"]["sog"]
    os_length = state["ownship"]["length"]

    state_collision_parameters = []

    for i in state["targetships"]:
        ts_pos = (i["xpos"], i["ypos"])
        ts_cog = i["cog"]
        ts_sog = i["sog"]
        ts_length = i["length"]

        collision_parameters = calculate_collision_parameters(os_pos, os_cog, os_sog, os_length, ts_pos, ts_cog, ts_sog, ts_length)
        state_collision_parameters.append(collision_parameters)

    return state_collision_parameters


def print_collision_parameters(coll_parameters):
    for j in range(len(coll_parameters)):
        print("Range: {:0.2f} || DCPA: {:0.2f} || TCPA: {:0.2f} || BCR: {:0.2f} || TBCR: {:0.2f} "
              "|| Q: {:0.2f} || Q1: {:0.2f} || v_rat: {:0.2f}"
